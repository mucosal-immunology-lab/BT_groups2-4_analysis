...
kraken2_preprocess <- function(filepath = NULL, otu_id_colname = '#OTU ID', lineage_colname = 'Consensus Lineage') {
  ...
  kraken2_input <- read_csv(file = filepath, skip = 1, show_col_types = FALSE)
  ...
  consensus_lineage_kraken2$species <- replace(consensus_lineage_kraken2$species, is.na(consensus_lineage_kraken2$species), 'sp')
